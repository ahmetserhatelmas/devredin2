...
const TURKEY_CITIES = {
  ...
  "Aydın": ["Bozdoğan","Buharkent","Çine","Didim","Efeler","Germencik","İncirliova","Karacasu","Karpuzlu","Koçarlı","Köşk","Kuşadası","Kuyucak","Nazilli","Nazilli","Söke","Sultanhisar","Yenipazar"],
  ...
  "Kastamonu": ["Abana","Ağlı","Araç","Azdavay","Bozkurt","Çatalzeytin","Daday","Devrekani","Doğanyurt","Hanönü","İhsangazi","İnebolu","Kastamonu Merkez","Küre","Pınarbaşı","Seydiler","Şenpazar","Taşköprü","Tosya"],
  ...
  "Sinop": ["Ayancık","Boyabat","Dikmen","Durağan","Erfelek","Gerze","İmranlı","Saraydüzü","Sinop Merkez","Türkeli"],
...
function populateCityDropdown(selectEl, defaultText = 'Tüm İller') {
    selectEl.innerHTML = `<option value="">${defaultText}</option>`;
    Object.keys(TURKEY_CITIES).sort().forEach(city => {
...
function populateDistrictDropdown(selectEl, cityName, defaultText = 'Tüm İlçeler') {
    selectEl.innerHTML = `<option value="">${defaultText}</option>`;
    selectEl.disabled = !cityName;
    if (!cityName || !TURKEY_CITIES[cityName]) return;
    TURKEY_CITIES[cityName].forEach(district => {
        const opt = document.createElement('option');
        opt.value = district;
